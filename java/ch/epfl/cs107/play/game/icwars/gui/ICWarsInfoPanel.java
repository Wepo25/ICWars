...
public class ICWarsInfoPanel implements Graphics {
    ...
    private final TextGraphics unitNameText, unitHealthText, unitDamageText, unitHealText;
    ...
    public ICWarsInfoPanel(float cameraScaleFactor) {
        ...
        unitHealthText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f,
                false, false, new Vector(0, -0.85f * fontSize - 0.5f),
                TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f, 3001f);

        unitDamageText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f,
                false, false, new Vector(0, -2.05f * fontSize - 0.4f),
                TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f, 3001f);

        unitHealText = new TextGraphics("", fontSize, Color.WHITE, null, 0.0f,
                false, false, new Vector(0, -3.0f * fontSize - 0.4f),
                TextAlign.Horizontal.LEFT, TextAlign.Vertical.MIDDLE, 1.0f, 3001f);
    }
    ...
    private void drawUnitDetails(Canvas canvas, float height, float width) {
        ...
        unitDamageText.setText("DMG: " + unit.getPower());
        unitDamageText.draw(canvas);
        if (unit.getName() == "Healer") {
            unitHealText.setRelativeTransform(characteristicsTransform);
            unitHealText.setText("HEAL: " + unit.getHeal());
            unitHealText.draw(canvas);
        }
    }
